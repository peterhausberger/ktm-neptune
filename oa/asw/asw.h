...
#define _SINE_OFFSET (0.48f - 0.15f) /* The offset parameter is determined by aligning the motor to phase A. The negative
...
static void _step_encoder(void) {
	...
	if (scheduler0.cycle % 4 == 0) { /* Every 2 ms. For up to 5000 rpm. @todo Rework! */
		float angular_velocity_radps = remainderf(asw.out.measured_angle - _previous_angle_for_rpm_calculation, 2 * PI) / 0.002f;
...
static void _step_currents(void) {
	...
	asw.out.iq_raw = (2.0 / 3.0) * (-sinf(angle - phi) * asw.out.ia -
